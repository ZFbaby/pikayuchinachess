...
void MainThread::search() {
  ...
  sync_cout << "bestmove " << UCI::move(bestThread->rootMoves[0].pv[0]);
    ...
    if (bestThread->rootMoves[0].pv.size() > 1 || bestThread->rootMoves[0].extract_ponder_from_tt(rootPos)){
        ...
        // PUSH iOS
        bestmove_to_ui(UCI::move(bestThread->rootMoves[0].pv[0]), UCI::move(bestThread->rootMoves[0].pv[1]));
        // POP iOS
    }else{
        // PUSH iOS
        // Important: Sending `nullptr` as the second parameter will cause **crash**. So, sending a string with zero size is better.
        bestmove_to_ui(UCI::move(bestThread->rootMoves[0].pv[0]), "");
        // POP iOS
    }
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              int opt = 126 * prev / (std::abs(prev) + 153);
          ...
          while (true)
          {
              ...
              bestValue = CCStockfish::search<Root>(rootPos, ss, alpha, beta, adjustedDepth, false);
